...
data modify storage lightium:main Item set value '[{"text":"une ","color":"white"},{"text":"Pickaxe Lv.5","color":"gold"},{"text":" !"}]'
execute if score #success lightium.data matches 1 run loot spawn ~ ~ ~ loot stardust:i/original_stardust_pickaxe
...
execute if score #success lightium.data matches 1 run data modify entity @e[type=item,sort=nearest,limit=1,nbt={Age:0s,Item:{tag:{stardust:{original:1b}}}}] Item.tag.Enchantments set value [{id:"efficiency",lvl:10}]
